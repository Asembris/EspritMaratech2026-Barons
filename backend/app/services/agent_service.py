...
class AgentService:
    ...
    def get_tools(self, current_user_id: int):
        ...
        @tool
        async def text_to_lsf(text: str) -> str:
            """Traduire un texte en Langue des Signes Tunisienne (LST/LSF).
            Retourne l'URL de la vidéo générée.
            Utile si l'utilisateur demande "comment on dit ... en signes" ou active le mode accessibilité.
            """
            from app.services.lsf_service import get_lsf_service
            service = get_lsf_service()
            try:
                # Agent tools are sync by default in this setup, but we need async
                # Langchain supports async tools if the agent is async.
                # Here we are inside a method. let's try to run it.
                # If this fails because of async, we might need a wrapper.
                # But let's assume the agent executor can handle it or we use run_in_executor if needed.
                # Actually, the method is defined as async def, Langchain handles it.
                
                result = await service.translate_text(text)
                if result.get("error"):
                    return f"Erreur de traduction : {result['error']}"
                
                video_url = f"/api/lsf/video/{os.path.basename(result['video_path'])}"
                glosses = ", ".join(result.get("glosses", []))
                return f"Voici la traduction en signes : {video_url} (Signes utilisés : {glosses})"
            except Exception as e:
                return f"Erreur technique lors de la traduction : {str(e)}"

        return [check_balance, get_transaction_history, search_product, check_product_stock_price, recommend_products_based_on_history, get_my_cart, get_cart_total, add_product_to_cart, remove_product_from_cart, transfer_money, check_product_price, checkout_cart, text_to_lsf]
