...
import { convertToGloss, ConvertResponse, convertTextToLSF } from '@/lib/api';
...
export default function SignConverter() {
    ...
    const handleConvert = async () => {
        ...
        try {
            ...
            // 1. Text -> LSF Video (New Pipeline)
            // 2. Text -> Gloss/Fingerspelling (Old Pipeline for details/fallback)

            const [lsfData, glossData] = await Promise.allSettled([
                convertTextToLSF(inputText),
                convertToGloss(inputText, useLlm)
            ]);

            // Handle LSF Video Result
            if (lsfData.status === 'fulfilled' && lsfData.value.video_url) {
                setVideoUrl(`${API_BASE_URL}${lsfData.value.video_url}`);
            }

            // Handle Gloss/Fingerspelling Result
            if (glossData.status === 'fulfilled') {
                setResult(glossData.value);
                const textToSpeak = glossData.value.summarized || glossData.value.gloss;
                announce(`Traduction terminée. ${textToSpeak}`);
            } else {
    ...
    const handleKeyDown = useCallback((e: KeyboardEvent) => {
        ...
        switch (e.key) {
            ...
            case ' ':
                e.preventDefault();
                if (!isPlaying && !videoUrl) playAnimation(); // Only anim fingerspelling if no video OR explicitly requested
    ...
    }, [result, isPlaying, announce, videoUrl]);
        ...
        >
            ...
            {(result || videoUrl) && (
                <>
                    ...
                    {/* VIDEO PLAYER SECTION (Priority) */}
                    {videoUrl && (
                        <section
                            aria-label="Vidéo LSF générée"
                            className="bg-gray-800 rounded-xl overflow-hidden mb-6 border border-purple-500/50 shadow-lg shadow-purple-900/20"
                        >
                            <h3 className="bg-gray-900/50 p-4 text-lg font-semibold text-purple-300 border-b border-gray-700">
                                🎥 Traduction Vidéo (LSF)
                            </h3>
                            <div className="aspect-video bg-black relative">
                                <video
                                    ...
                                    onPlay={() => announce("Lecture de la vidéo")}
                                    onPause={() => announce("Vidéo en pause")}
                                />
                            </div>
                        </section>
                    )}
                    ...
                    {/* Fallback / Detail: Fingerspelling */}
                    {/* Only show if NO video (Fallback Mode) OR if user wants to dig deeper? 
                        The prompt asked for "fallback if we cant compose the video". 
                        However, having fingerspelling available below the video is often useful as a supplement.
                        For strict "fallback" behavior: { !videoUrl && result && (...) }
                        But I'll leave it visible as it provides the fingerspelling breakdown which the compiled video might skip or do differently.
                        Actually, let's make it collapsible or just show it below. I'll show it below.
                    */}
                    {result && (
                        <section
                            ref={mainDisplayRef}
                        ...
                        >
                            <h3 id="spell-heading" className="text-lg font-semibold text-pink-400 mb-4 text-center">
                                {videoUrl ? "🔤 Détails : Épellation" : "🤟 Épellation en Images (Fallback)"}
                            </h3>
                            {/* ... Content stays same ... */}
